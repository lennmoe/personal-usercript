...
`);
  ...
  // --- FIX: GM_config.init() reads saved settings asynchronously (it awaits
  // GM_getValue internally). If we call GM_config.get() before that finishes,
  // config.fields[name] is still undefined and .value throws. We wait for the
  // 'init' event before using GM_config.get() anywhere below.
...
`,
    events: {
      init: function () {
        // Fires once config.fields is actually populated - safe to call GM_config.get() after this.
        resolveConfigReady();
      },
      save: function () {
        console.log('saved');
  ...
  // --- FIX: wait until GM_config has finished loading saved settings before
  ...
  [...document.querySelectorAll('article.vnreleases > details')].forEach((detail) => {
    ...
    for (const release of releases) {
      ...
      if (!unofficial && !mtl) {
        ...
        const officialLinkIcon = release.querySelector('abbr.icon-external[title="Official website"]');
        if (officialLinkIcon) {
        ...
        const otherLinks = [...release.querySelectorAll('.tc_links ul li > a')];
        if (otherLinks.length > 0) {
          otherLinks.forEach((linkAnchor) => {
            if (!info.links[linkAnchor.href]) {
              info.links[linkAnchor.href] = { type: linkAnchor.textContent.trim(), title: releaseTitle };
...
<a href="${link}" title="${displayLink}">
  ${displayLink}${linkPrice ? ` <span>${linkPrice}</span>` : ''}
</a>`;
        if (linkType === 'Official website') {
  ...
  try {
    ...
    if (data.success) {
      ...
      if (usa) {
        ...
        return (
          data.data.price_overview.final_formatted + ' (' + (await getSteamPrice(link, true)) + ')'
        );
      }
    }
  } catch (error) {
