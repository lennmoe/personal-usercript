...
if (document.URL.match(/v\d+(#main)?$/)) {
  // VNDB's page structure changed: the title's wrapper div and the "X votes total"
  // phrasing are gone. Fall back through a few selectors and a looser regex so this
  // keeps working even if the markup shifts slightly again.
  const titleElem =
    document.querySelector('#maincontent > div > h1') ||
  ...
  // .votegraph's <tfoot><td> now reads e.g. "1876 votes (rank 171)6.98 average (rank 4382)"
  const voteGraphFootTd = document.querySelector('.votegraph tfoot td');
  const voteCountMatch = voteGraphFootTd
    ? voteGraphFootTd.textContent.match(/([\d,]+)\s*votes/)
    : null;
  const voteCount = voteCountMatch ? parseInt(voteCountMatch[1].replace(/,/g, '')) : 0;

  if (!voteGraphFootTd || !voteCount) {
    console.log('VNDB Score Graph: could not find vote stats on this page, skipping.');
  } else {

  // list of released releases with date, lang, title and remove duplicates
  const vnReleasesElem = document.querySelector('article.vnreleases');
  if (!vnReleasesElem) {
    console.log('VNDB Score Graph: could not find article.vnreleases on this page.');
  }
  const releases = (vnReleasesElem
    ? [...vnReleasesElem.querySelectorAll('tr')]
        .map((tr) => {
          const rel = {};
          rel.lang = [
  ...
  const voteStatsElem = voteGraphFootTd;
...
async function getVotes(voteCount, vnID, displayText) {
  ...
  for (let i = 1; i <= last; i++) {
    ...
    // VNDB's votes list is now <article class="browse votelist"><table>...
    const votelistElem = doc.querySelector('.browse.votelist') || doc.querySelector('.mainbox.votelist');
    const votelistBody = votelistElem ? votelistElem.querySelector('tbody') : null;
    const voteRows = votelistBody ? [...votelistBody.querySelectorAll('tr')] : [];
    if (voteRows.length === 0) last = true;
    else {
      votes.push(
        ...voteRows.map((tr) => {
          const vote = {};
